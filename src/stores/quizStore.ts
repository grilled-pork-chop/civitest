...
export const quizActions = {
  ...
  answerQuestion: (questionIndex: number, choiceIndex: number) => {
    appStore.setState((state) => {
      ...
      // Answers are only recorded during a live quiz, where every question is a
      // ShuffledQuestion; the guard keeps this type-safe without a cast.
      const isCorrect =
        'shuffledChoices' in question
          ? (question.shuffledChoices[choiceIndex]?.isCorrect ?? false)
          : false;
  ...
  endQuiz: (): QuizResult | null => {
    ...
    // Only topics that appear in this quiz, kept in canonical TOPICS order.
    const topicStats = aggregateTopicStats(quiz.questions, quiz.answers);
    const topicPerformance: TopicPerformance[] = TOPICS.filter(
      (topic) => topicStats[topic.id]
    ).map((topic) => ({ topicId: topic.id, ...topicStats[topic.id] }));

...
import { PASS_PERCENTAGE, QUIZ_CONFIG } from '@/types';
import { useThemeColors } from '@/theme/useTheme';
import { haptics } from '@/services/haptics';
import { cn } from '@/lib/utils';

const PASS_THRESHOLD = PASS_PERCENTAGE;
const TIME_LIMIT_MINUTES = QUIZ_CONFIG.timeLimit / 60;
          ...
          <View className="px-5 pb-6">
            ...
            {hasResults ? (
              ...
              <View className="items-center">
                ...
                <HeroFacts count={questions?.length || 0} />
              </View>
            ) : (
...
/** The three exam facts shown under the hero CTA (shared by both hero states). */
function HeroFacts({ count }: { count: number }) {
  return (
    <View className="flex-row flex-wrap gap-x-5 gap-y-2 mt-6">
      <Fact icon={<BookOpen size={16} color="#ffffff" />} text={`${count} questions`} />
      <Fact icon={<Clock size={16} color="#ffffff" />} text={`${TIME_LIMIT_MINUTES} minutes`} />
      <Fact icon={<Target size={16} color="#ffffff" />} text={`${PASS_THRESHOLD}% requis`} />
    </View>
  );
}

...
/**
 * A question as persisted inside a saved quiz result. It carries the extra
 * `shuffledChoices` / `originalToShuffledMap` fields produced at quiz time so
 * that reviewing a historical quiz renders the choices in the exact order they
 * were shown; otherwise the stored `selectedChoiceIndex` (an index into the
 * shuffled order) would point at the wrong option once the fields are stripped.
 * Both are optional for backward-compatibility with quizzes saved before this.
 */
export const ResultQuestionSchema = QuestionSchema.extend({
  shuffledChoices: z.array(ChoiceSchema).optional(),
  originalToShuffledMap: z.array(z.number()).optional(),
});
